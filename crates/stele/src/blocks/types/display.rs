...
use std::pin::Pin;
const DEFAULT_OUTPUT_KEY: &str = "display_message";
...
impl BlockBehaviour for DisplayBlock {
    fn id(&self) -> &str {
        &self.base.id
    }
    ...
    {
        Box::pin(async move {
            let message = self.base.get_required_string("message")?;
            let target = self
                .base
    ...
    fn validate(&self) -> Result<(), BlockError> {
        self.base.get_required_string("message")?;
