...
#[tokio::test]
async fn quic_provider_concurrency_threshold_uni() {
            ...
            if let Ok(conn) = incoming.await {
                tokio::spawn(async move {
                    ...
                    if let Ok(mut recv) = conn.accept_uni().await {
                        if let Ok(_req) = recv.read_to_end(64 * 1024).await {
    ...
    let req_timeout_ms: u64 = std::env::var("STEEL_QUIC_REQ_TIMEOUT_MS")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or(3000);
    async fn run_attempt(
        p: Arc<QuicExchangeProvider>,
        parallel: usize,
        req_timeout_ms: u64,
    ) -> (usize, Vec<u128>) {
            ...
            handles.push(tokio::spawn(async move {
                ...
                loop {
                    ...
                    match tokio::time::timeout(
                        Duration::from_millis(req_timeout_ms),
                        pr.exchange_data(payload.clone()),
                    )
                    .await
                    {
                        Err(_elapsed) => {
        ...
        for h in handles {
            match h.await {
                Ok(Ok(d)) => latencies.push(d.as_millis()),
                _ => errors += 1,
            }
        }
        (errors, latencies)
    ...
    let mut cur = 4usize;
    println!("THRESHOLD_SEARCH_UNI start base={cur} max_cap={max_cap}");
    while cur <= max_cap {
        let (errs, lats) = run_attempt(provider.clone(), cur, req_timeout_ms).await;
        println!(
            "ATTEMPT_UNI parallel={cur} errors={errs} latency={}",
            summarize(&lats)
        );
        if errs == 0 {
            last_ok = cur;
    ...
    if first_fail == max_cap + 1 {
        println!("RESULT_UNI no failure observed up to max_cap={max_cap} (last_ok={last_ok})");
        assert!(last_ok >= 4, "unexpectedly low concurrency threshold uni");
        return;
    }
    ...
    if last_ok + 1 < first_fail {
        ...
        while low <= high {
            let mid = (low + high) / 2;
            let (errs, lats) = run_attempt(provider.clone(), mid, req_timeout_ms).await;
            println!(
                "BINARY_ATTEMPT_UNI mid={mid} errors={errs} low={low} high={high} latency={}",
                summarize(&lats)
            );
            if errs == 0 {
                last_ok = mid;
    ...
    println!("RESULT_UNI max_reliable={last_ok} first_fail={first_fail}");
    assert!(
        last_ok >= 4,
        "max reliable uni concurrency too low: {last_ok}"
    );
}
